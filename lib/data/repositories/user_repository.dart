...
class UserRepository {
  ...
  double _calculateDistance(
    ...
    final a =
        ...
        _cos(lat1) * _cos(lat2) * _sinHalf(dLon) * _sinHalf(dLon);
    final c = 2 * (a < 1.0 ? _asinSqrt(a) : _acos(1.0));

    return earthRadius * c;
  }

  double _toRadians(double degrees) => degrees * (3.1415926535897932 / 180);
  double _sinHalf(double value) =>
      value.sin() * 0.5 + 0.5; // Simplified
  ...
  double _asinSqrt(double x) {
    // Simplified arcsin using Taylor series approximation
    double result = 0.0;
    double term = x;
    for (int i = 0; i < 20; i++) {
      result += term;
      term *= (2 * i + 1) * x * x / (2 * (i + 1) * (2 * i + 3));
    }
    return result;
  }
}

/// Extension to add sin to double
